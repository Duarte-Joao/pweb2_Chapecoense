...
<div class="max-w-lg mx-auto bg-white rounded shadow p-6">
    <h1 class="text-2xl font-bold text-green-700 mb-6">Editar Partida</h1>
    ...
    <form action="{{ route('partidas.update', $partida->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="adversario" value="{{ old('adversario', $partida->adversario) }}"
                   class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500">
        </div>

        <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 mb-1">Data da Partida *</label>
            <input type="date" name="data_partida"
                   value="{{ old('data_partida', \Carbon\Carbon::parse($partida->data_partida)->format('Y-m-d')) }}"
                   class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500">
        </div>

        <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 mb-1">Estádio *</label>
            <input type="text" name="estadio" value="{{ old('estadio', $partida->estadio) }}"
                   class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500">
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="competicao"
                ...
                @foreach(['Brasileirão Série A','Brasileirão Série B','Copa do Brasil','Copa Sul-Americana','Copa Libertadores','Campeonato Catarinense','Amistoso'] as $comp)
                    <option value="{{ $comp }}" {{ old('competicao', $partida->competicao) == $comp ? 'selected' : '' }}>{{ $comp }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="number" name="gols_chapecoense" value="{{ old('gols_chapecoense', $partida->gols_chapecoense) }}" min="0"
                       class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Gols Adversário *</label>
                <input type="number" name="gols_adversario" value="{{ old('gols_adversario', $partida->gols_adversario) }}" min="0"
                       class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500">
            </div>
        </div>

        {{-- SELECT puxando dados da tabela jogadores, já selecionando o atual --}}
        <div class="mb-6">
            ...
            <select name="jogador_id"
                ...
                @foreach($jogadores as $jogador)
                    <option value="{{ $jogador->id }}"
                        {{ old('jogador_id', $partida->jogador_id) == $jogador->id ? 'selected' : '' }}>
        ...
        <div class="flex gap-3">
            <button type="submit" class="bg-green-700 text-white px-6 py-2 rounded hover:bg-green-800">Atualizar</button>
