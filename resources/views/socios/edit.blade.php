...
<div class="max-w-lg mx-auto bg-white rounded shadow p-6">
    <h1 class="text-2xl font-bold text-green-700 mb-6">Editar Sócio</h1>
    ...
    <form action="{{ route('socios.update', $socio->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="nome" value="{{ old('nome', $socio->nome) }}"
        ...
        <div class="mb-4">
            ...
            <input type="text" name="cpf" value="{{ old('cpf', $socio->cpf) }}" maxlength="11"
                   class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500">
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="telefone" value="{{ old('telefone', $socio->telefone) }}"
                   class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500">
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="categoria_socio"
                ...
                @foreach(['Geral', 'Cadeira Lateral', 'Cadeira Central'] as $cat)
                    <option value="{{ $cat }}" {{ old('categoria_socio', $socio->categoria_socio) == $cat ? 'selected' : '' }}>{{ $cat }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-6">
            ...
            <input type="date" name="data_associacao"
                   value="{{ old('data_associacao', \Carbon\Carbon::parse($socio->data_associacao)->format('Y-m-d')) }}"
                   class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500">
        </div>
